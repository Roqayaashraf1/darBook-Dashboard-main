    ...
    function dispatch(stream, state) {
      ...
      if (!isPHP) {
        if (stream.match(/^<\?\w*/)) {
          ...
          if (!state.html) state.html = CodeMirror.startState(phpMode, htmlMode.indent(state.html, "", ""))
        ...
        return style;
      } else if (isPHP && state.html.tokenize == null && stream.match("?>")) {
        state.curMode = htmlMode;
        state.curState = state.html;
        if (!state.html.context.prev) state.html = null;
        return "meta";
      } else {
    ...
    return {
      ...
      copyState: function(state) {
        var html = state.html, htmlNew = CodeMirror.copyState(htmlMode, html),
            php = state.html, phpNew = php && CodeMirror.copyState(phpMode, php), cur;
